...
void port::dump(libjson::ofstream& os) const
{
    os << libjson::stream_marker::BEGIN_STRUCTURE;
    os << libjson::make_pair("name", name());
    os << libjson::make_pair("width", width());
    os << libjson::make_pair("depth", depth());
    os << libjson::make_pair("direction", to_string(_direction));
